...
from mujoco_exp.memory import ReplayBuffer
from rltorch.memory import MultiStepMemory, PrioritizedMemory
from torch.optim import Adam
from tqdm import tqdm

from mujoco_exp.models import PPOActor, SACActor, TwinnedQCritic, VCritic
...
class PPO:
    def __init__(self, env, n_steps, local_dir=None, env_name="None"):
        self.env = env
    ...
    def run(self):
        ...
        f = open(os.path.join("results", self.env_name+"_PPO"+".csv"), 'w')
        f.write("mean_return, std, steps, episodes\n")
        f.close()
        ...
        while True:
            for t in tqdm(range(self.traj_len)):
                ...
                next_state_, reward, terminated, truncated, _ = self.env.step(
                    action.cpu().numpy())
    ...
    def evaluate(self):
        ...
        f = open(os.path.join("results", self.env_name+"_PPO"+".csv"), 'a')
...
class SAC:
    def __init__(self, env, entropy_tuning=True, per=False, n_steps=3000000, local_dir=None, env_name="None"):
        self.env = env
    ...
    def run(self):
        f = open(os.path.join("results", self.env_name+"_SAC"+".csv"), 'w')
        f.write("mean_return, std, steps, episodes\n")
        f.close()
        while True:
            self.train_episode()
    ...
    def evaluate(self):
        ...
        mean_return = float(sum(returns)) / float(episodes)
        std = np.array(returns).std()
        f = open(os.path.join("results", self.env_name+"_SAC"+".csv"), 'a')
        if mean_return > self.best_reward:
            torch.save(self.policy.state_dict(), os.path.join(
                self.model_dir, 'policy.pth'))
            torch.save(self.critic.state_dict(), os.path.join(
