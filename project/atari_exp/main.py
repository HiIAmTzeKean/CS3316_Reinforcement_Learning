...
from atari_exp.atari_wrappers import make_atari, wrap_deepmind
from atari_exp.memory import ReplayMemory
from atari_exp.models import DQN, Agent, dueling_DQN
...
def evaluate(step, policy_net, env, n_actions, eps=0.01, num_episode=5,model='DQN'):
    best_reward = 0.0
    ...
    for _ in range(num_episode):
        ...
        e_reward = 0
        for _ in range(5):
            n_frame, _, terminated, truncated, _ = env.step(0)
        ...
        while not done:
            ...
            n_frame, reward, terminated, truncated, _ = env.step(action)
            done = terminated or truncated
            n_frame = fp(n_frame)
            q.append(n_frame)
    ...
    f = open(os.path.join("results", env_name+"_"+model+".csv"), 'a')
    ...
    if avg_reward > best_reward:
        torch.save(policy_net.state_dict(), "models/" +
                   env_name+"_"+model+"_"+str(int(avg_reward))+".pth")
        best_reward = avg_reward
    f.write("%f, %f, %d\n" % (avg_reward, std, step))
    f.close()

def init_log_file(env_name,model):
    f = open(os.path.join("results", env_name+"_"+model+".csv"), 'w')
    f.write("avg_reward, std, step\n")
    f.close()
...
def parse_arguments():
    """Argument parser for atari"""
    parser = argparse.ArgumentParser()
    parser.add_argument("--env_name", type=str,
                        choices=["BreakoutNoFrameskip-v4", "PongNoFrameskip-v4",
                                 "VideoPinball-ramNoFrameskip-v4", "BoxingNoFrameskip-v4"],
                        default='BreakoutNoFrameskip-v4')
    parser.add_argument("--is_dueling", type=str, default='False', choices=['True', 'False'])
    args, extra = parser.parse_known_args()
    return args

# allow the run.py to call this as import
...
# get the observation from reset only
c, h, w = fp(env.reset()[0]).shape
n_actions = env.action_space.n

if args.is_dueling=='True':
    model="DDQN"
...
for step in tqdm(range(NUM_STEPS)):
    if done:
        ...
        for i in range(5):
            n_frame, reward, _, _, _ = env.step(0)
            n_frame = fp(n_frame)
            q.append(n_frame)
            rq.append(reward)
    ...
    state = torch.cat(list(q))[1:].unsqueeze(0)
    action, eps = agent.select_action(state, train)
    n_frame, reward, terminated, truncated, info = env.step(action)
    done =  terminated or truncated
    n_frame = fp(n_frame)
    ...
    if step % EVALUATE_FREQ == 0:
        evaluate(step, policy_net, env_raw,
                    n_actions, eps=0.1, num_episode=20,model=model)
